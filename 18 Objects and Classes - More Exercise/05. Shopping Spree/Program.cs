...
namespace ShoppingSpree
{
    class Program
    {
        static void Main(string[] args)
        {
                    ...
                    if (item.Name == name)
                    {
                        ...
                        foreach (var item1 in peopleList)
                        {
                            if (item1.BagOfProducts.Name == product)
                            {
                                ...
                                {
                                    Console.WriteLine($"{item.Name} bought {item.BagOfProducts.Name}");
            ...
            foreach (var item in peopleList)
            {
                ...
                {
                    Console.WriteLine($"{item.Name} - {string
                        .Join(", ", item.ProductsBought)}");
                }
                else
                {
    ...
    class Person
    {
        ...
        public Person(string name,
            double money, Product bagOfProducts)
        {
            this.Name = name;
            this.Money = money;
